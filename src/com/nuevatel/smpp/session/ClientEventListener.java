 ...
 */
public class ClientEventListener implements ServerPDUEventListener {
    /*private variables*/
     ...
     */
    public ClientEventListener(ClientSession clientsession, int threadPoolSize){
                ...
                if (commandID == Data.ENQUIRE_LINK){
                    ...
                    if (clientSession.getState()==Data.BIND_RECEIVER || clientSession.getState()==Data.BIND_TRANSCEIVER){
                        ...
                        if (deliverSM.getEsmClass()==4 || deliverSM.getEsmClass()==8){
                            //delivery receipt
                            getThreadPool().submit(new DeliveryReceiptMCDispatcher(pdu, clientSession));
                        }
                        else {
                            //short message
                            getThreadPool().submit(new MCDispatcher(pdu, clientSession));
                        }
                    }
                    else {
                        response = request.getResponse();
